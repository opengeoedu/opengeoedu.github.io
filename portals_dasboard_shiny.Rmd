...
flext <- spanFlexTableRows(flext, j = "Typ", 1,4)
flext <- spanFlexTableRows(flext, j = "Typ", 5,8)
flext[,1, newpar = TRUE] <- paste0("(",sapply(summary(portale$Typ), function(x) rep(x,4)),")")

flext[,2] <- paste0(" (",as.numeric(rowSums(as.matrix(ftab))),")")
flext <- addFooterRow(flext, paste0("Datenportale insgesamt: ",dim(portale)[1]), colspan = 6)
flext <- setZebraStyle(flext, even = '#eeeeee', odd = 'white' )
