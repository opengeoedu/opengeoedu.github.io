...
ftab <- ftable(data.frame(Typ = Typ, Land = Land, Bezug = portale$Bezug))
...
tags$div(align="middle", style="text-align:center",
 bscols(widths = c(6,6),
        div(style="display:inline-block; text-align:left",title="Aktivierte Datenfilter sind sowohl auf der Karte als auch in der Tabelle wirksam", filter_checkbox("bezug_portal", "Portal-Art", sd, ~Typ, inline = FALSE)),
        div(style="display:inline-block; text-align:left",title="Aktivierte Datenfilter sind sowohl auf der Karte als auch in der Tabelle wirksam", filter_checkbox("bezug_check", "Räumlicher Bezug", sd, ~Bezug, inline = FALSE))
   )
 )
...
shinyApp(
  ui = fluidPage(
      ...
      radioButtons("Typ", "Art des Portals:",
          choiceNames = 
            list("Open Data Portal","GDI / Geoportal"), 
          choiceValues = 
            list("odp","gdi"),
          inline = TRUE,
      ),
